    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0f172a] text-slate-900 dark:text-slate-100 transition-colors duration-300 flex flex-col font-['Noto_Sans_Arabic'] pb-24 md:pb-32">
        ...
        <div className="max-w-6xl mx-auto w-full flex items-center justify-between">
          <div className="flex items-center gap-2 md:gap-3">
            <button 
              ...
              className="w-10 h-10 bg-gradient-to-tr from-blue-600 to-indigo-600 rounded-2xl flex items-center justify-center text-white font-bold shadow-lg shadow-blue-500/20 text-xl active:scale-95 transition-transform"
            >
              ق
            </button>
            <div className="hidden xs:block">
              <h1 className="text-lg md:text-xl font-black bg-clip-text text-transparent bg-gradient-r from-blue-700 to-indigo-700 dark:from-blue-400 dark:to-indigo-400 cursor-pointer" onClick={() => setCurrentView('templates')}>
                قوالب الدعم
              </h1>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1.5 md:gap-3">
            <button 
                onClick={() => setCurrentView(prev => prev === 'guide' ? 'templates' : 'guide')}
      ...
      <main className="max-w-6xl mx-auto w-full px-3 md:px-8 pb-3 md:pb-8 flex flex-col">
        {currentView === 'templates' ? (
          ...
          <GuidePage onBack={() => setCurrentView('templates')} />
        )}
      </main>
      ...
      <button 
        onClick={scrollToTop}
        className={`fixed left-6 bottom-[100px] md:bottom-[120px] z-[80] w-14 h-14 rounded-2xl bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 shadow-2xl flex items-center justify-center text-slate-500 dark:text-slate-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all duration-300 active:scale-90 ${
          ...
          <div className="relative flex-1">
            <input 
              ...
              onChange={(e) => {
                ...
                if (currentView !== 'templates') setCurrentView('templates');
              }} 
              placeholder="بحث سريع..." 
              className="w-full h-12 pr-10 pl-4 rounded-2xl border-0 bg-slate-100 dark:bg-slate-900/50 text-slate-900 dark:text-white placeholder-slate-400 focus:ring-2 focus:ring-blue-500/50 outline-none font-medium text-sm transition-all" 
            />
